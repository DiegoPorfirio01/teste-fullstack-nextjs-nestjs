...
test.describe("Dashboard", () => {
  test("exibe sidebar com navegação", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /transações/i }).first()).toBeVisible();
    await expect(page.getByRole("link", { name: /comprar crédito/i }).first()).toBeVisible();
  });
  ...
  test("página de transações carrega", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/transactions/);
    await expect(page.getByRole("heading", { name: /transações/i })).toBeVisible({ timeout: 10000 });
  });
  ...
  test("usuário autenticado em /auth/login é redirecionado para dashboard", async ({ page }) => {
